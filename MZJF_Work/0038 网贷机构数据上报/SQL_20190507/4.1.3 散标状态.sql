...
SELECT
    ...
    date_format(bar.create_time,'yyyy-MM-dd hh:mm:ss')                                     productDate,        --5.状态更新时间
    "",
    ""
from 
...
select
    ...
    date_format(c.productDate,'yyyy-MM-dd hh:mm:ss')                                        productDate,        --5.状态更新时间
    "",
    ""
from 
    (select distinct bo_id from odsopr.debt_exchange_account_20190430 where bank_flag = 1 and price > 0) t
left join
    ...
    from 
        (--放款后首次还款中状态
        select
            concat(date_add(coalesce(bo_success_time,bo_finish_time),1)," 00:00:00") productDate,
            bo_id,
            '' br_time,
            '' br_repay_time,
            '5' productStatus,
        ...
        from
            ...
            --逾期时，取应还时间为上报时间，状态逾期
            select 
                concat(cast(date_add(br.br_time,1) as STRING)," 00:00:00") productDate,
                br.bo_id, 
                br.br_time, 
                NULL br_repay_time
            from idw.fact_borrows_repayment br
            ...
            select 
                date_add(coalesce(cast(to_date(br_repay_time) as STRING),concat(br_time," 00:00:00")),1) productDate,
                br.bo_id, 
                br.br_time, 
                to_date(br.br_repay_time) br_repay_time
            from idw.fact_borrows_repayment br
