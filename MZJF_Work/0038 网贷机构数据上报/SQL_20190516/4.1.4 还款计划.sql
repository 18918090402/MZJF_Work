...
SELECT
    ...
    printf("%.2f",br.br_price_b*1.0)                                            curFund,                --8.本条记录（当期）应还本金（元）
    printf("%.2f",br.br_price_l*1.0)                                            curInterest,            --9.本条记录（当期）应还利息（元）
    printf("%.2f",(br_service_fee + price_return - br_repayed_tech_fee + 
    br_extra_fee + br_service_fee_other + br_post_consult_fee)*1.0)             curServiceCharge,       --10.本条记录（当期）应还服务费（元）
