set mapreduce.job.name = msc_report_01_userinfo; 
...
--出借人   --35456
drop table if exists msc.tmp_report_01_invest_user ;

create table msc.tmp_report_01_invest_user
as 
	select distinct t.user_id from odsopr.debt_exchange_account_20190430 t
	 where bank_flag = 1 
	   and price > 0 
	   and status in (1,2,3)
...
--借款人   --237858
drop table if exists msc.tmp_report_01_loan_user ;
...
insert overwrite table msc.report_01_userinfo
...
SELECT
    ...
    if(length(p2p.mobile) = 11,certPhoneHashSalt(p2p.mobile)['phone'],'-1')                                 userPhoneHash,          --12.手机号hash值
    if(length(p2p.mobile) = 11,certPhoneHashSalt(p2p.mobile)['salt'],'-1')                                  userUuid,               --13.用户手机盐值
