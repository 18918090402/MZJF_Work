...
CREATE TABLE IF NOT EXISTS `norm` (
  ...
  `create_time` datetime DEFAULT NULL COMMENT '创建时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='check表';
...
(SELECT * FROM (select sourceProductCode, totalIssue, sendTime from tmp_repayplan_init GROUP BY sourceProductCode,sendTime ORDER BY sendTime DESC) t3 GROUP BY sourceProductCode) t2
...
SELECT sourceCode, sourceProductCode, CAST(SUM(transMoney) AS DECIMAL(32,16)) transMoney
FROM tmp_transact 
...
FROM tmp_scatterinvest_3_status t1 LEFT JOIN tmp_transact t2
on t1.sourceProductCode = t2.sourceProductCode and t1.sourceCode = t2.sourceCode and t2.transType = '1'
...
SELECT sourceCode, sourceProductCode, CAST(SUM(curFund) AS DECIMAL(32,16)) + CAST(SUM(curInterest) AS DECIMAL(32,16)) AS total_sum
FROM last_repayplan WHERE repayTime < NOW()
group by sourceProductCode;


CREATE TEMPORARY TABLE transact_repay
SELECT sourceCode, sourceProductCode, CAST(SUM(transMoney) AS DECIMAL(32,16)) AS total_repay, MAX(SUBSTR(transTime, 1 ,10)) AS last_day
...
SELECT a.sourceCode, a.sourceProductCode, CAST(SUM(a.total_sum) AS DECIMAL(32,16)) AS replan_money,CAST(SUM(b.total_repay) AS DECIMAL(32,16)) AS transact_money,b.last_day AS last_day
...
ON t1.sourceProductCode = t2.sourceProductCode WHERE t2.sourceProductCode IS NOT NULL GROUP BY t1.sourceProductCode) AS `l`
LEFT JOIN (SELECT DISTINCT sourceProductCode FROM tmp_repayplan) r ON l.sourceProductCode = r.sourceProductCode
...
SELECT t1.sourceCode, t1.sourceProductCode FROM (SELECT * FROM tmp_transact where transType = '18') AS t1
LEFT JOIN tmp_repayplan AS t2 on t1.sourceProductCode = t2.sourceProductCode
WHERE t2.sourceProductCode IS NOT NULL AND t2.curFund > 0 AND t2.curInterest > 0 GROUP BY t1.sourceProductCode;
...
SELECT t1.sourceCode, t1.sourceProductCode, t1.amount, CAST(SUM(t2.invAmount) AS DECIMAL(32,16)) invAmount, CAST(SUM(t2.redpackage) AS DECIMAL(32,16)) redpackage, COUNT(1) count
...
COUNT(1) as count, NOW() as create_time 
FROM temp_scat_cred t1 WHERE CAST((amount - (invAmount + redpackage)) AS DECIMAL(32,16)) > 5.00;
...
SELECT t1.sourceProductCode, CAST((t1.loan + t2.loan_cast) AS  DECIMAL(32,16)) AS loan_money
FROM tmp_6 AS t1, 
...
SELECT t1.sourceProductCode, CAST((t1.invMoney + t2.redpackage) AS DECIMAL(32,16)) AS money
FROM tmp_7 AS t1, 
...
FROM tmp_success_transferStatus t1
LEFT JOIN tmp_undertakeinfo t2 ON t1.finClaimId = t2.unFinClaimId WHERE t1.fromType = '2' AND t2.unFinClaimId IS NULL GROUP BY t1.finClaimId) as t1;
