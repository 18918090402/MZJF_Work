...
drop table if exists report_kpis.tmp_repayplan_1;
create table report_kpis.tmp_repayplan_1 as
...
select *
 from
  (select *,
      row_number() over(partition by sourceProductCode order by repayTime desc) as rn
    from report_kpis.tmp_repayplan_1) a
where a.rn = 1;
...
SELECT t1.*, IFNULL(cast(t2.amount as decimal(38,6)),0) amount FROM report_kpis.creditor_transfer t1
