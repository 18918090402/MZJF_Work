...
SELECT "CERT20190411026" AS sourceCode, 'scatterinvest' AS intface_type, cast(COUNT(DISTINCT st.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'transact' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'transferproject' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'undertakeinfo' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'creditor' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'repayplan' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" sourceCode, 'userinfo_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'creditor_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_creditor 
WHERE userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'repayplan_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_repayplan WHERE userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'scatterinvest_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_scatterinvest WHERE userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'transact_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_transact WHERE transtype NOT IN ('3', '13', '27', '30') AND userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'transferproject_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_transferproject WHERE userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'undertakeinfo_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
-- 产品用户check
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'lendproductconfig' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'lendparticulars' AS intface_type, cast(COUNT(DISTINCT tp.userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'lendproductconfig_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
FROM report_check.tmp_lendproductconfig WHERE userIdcardHash IN ('-1', '0', '-')
;

INSERT INTO report_check.leak_user
SELECT "CERT20190411026" AS sourceCode, 'lendparticulars_leakypush' AS intface_type, cast(COUNT(userIdcardHash) as string) AS leak_user_data, to_date(NOW()) AS create_time 
...
SELECT sourceCode, sourceProductCode, sum(CAST(transMoney AS DECIMAL(32,16))) transMoney 
...
as 
select * from report_check.tmp_repayplan ;
...
SELECT sourceCode, sourceProductCode, sum(CAST(curFund AS DECIMAL(32,16))) + sum(CAST(curInterest AS DECIMAL(32,16))) AS total_sum 
...
SELECT sourceCode, sourceProductCode, sum(CAST(transMoney AS DECIMAL(32,16))) AS total_repay, MAX(SUBSTR(transTime, 1 ,10)) AS last_day 
...
SELECT a.sourceCode, a.sourceProductCode, sum(CAST(a.total_sum AS DECIMAL(32,16))) AS replan_money, sum(CAST(b.total_repay AS DECIMAL(32,16))) AS transact_money, b.last_day AS last_day 
...
-- 修改后
DROP TABLE IF EXISTS report_check.repay_nomoal_1;
create table report_check.repay_nomoal_1 as 
select t1.sourceproductcode
from report_check.tmp_repayplan t1, report_check.transact_repay_time t2 
where t1.replanId = t2.replanId
AND t2.transTime > t1.repayTime AND t1.sourceProductCode = t2.sourceProductCode ;


--SELECT 
--  t1.sourceProductCode 
--FROM 
--  report_check.tmp_repayplan t1, 
--  report_check.transact_repay_time t2 
--WHERE 
--  t1.replanId = CONCAT(
--    split_part(t2.replanId, '-', 1), 
--    '-', 
--    cast(
--      cast(
--        reverse(
...
LEFT JOIN (SELECT DISTINCT sourceProductCode FROM report_check.tmp_repayplan) r ON l.sourceProductCode = r.sourceProductCode 
...
cast(COUNT(DISTINCT t1.sourceProductCode) as string) AS count, 
to_date(NOW()) AS create_time
...
CREATE TABLE report_check.tmp_1 
as 
SELECT t1.sourceCode, t1.sourceProductCode FROM (SELECT * FROM report_check.tmp_transact where transType = '18') AS t1
LEFT JOIN report_check.tmp_repayplan AS t2 on t1.sourceProductCode = t2.sourceProductCode
WHERE t2.sourceProductCode IS NOT NULL AND cast(t2.curFund as decimal(32,16)) > 0 AND cast(t2.curInterest as decimal(32,16)) > 0  
...
SELECT t1.sourceCode, t1.sourceProductCode, t1.surplusAmount, sum(CAST(t2.invAmount AS DECIMAL(32,16))) invAmount, sum(CAST(t2.redpackage AS DECIMAL(32,16))) redpackage, COUNT(1) count 
FROM report_check.tmp_scatterinvest_3_status t1, report_check.tmp_creditor t2 
WHERE t1.sourceCode=t2.sourceCode AND t1.sourceProductCode=t2.sourceProductCode GROUP BY t1.sourceCode, t1.sourceProductCode, t1.surplusAmount;
...
SELECT *
FROM report_check.temp_scat_cred t1 WHERE (cast(surplusAmount as decimal(32,16)) - (cast(invAmount as decimal(32,16)) + cast(redpackage as decimal(32,16)))) > 5.00 LIMIT 10;
...
SELECT "CERT20190411026" as sourceCode, 
       ...
       '0' AS count, 
       to_date(now()) AS create_time 
FROM (select count(*) from  report_check.tmp_5) AS t1 ;
...
SELECT t1.sourceProductCode, (cast(t1.loan as decimal(32,16)) + cast(t2.loan_cast as decimal(32,16))) AS loan_money 
...
SELECT t1.sourceProductCode, (cast(t1.invMoney as decimal(32,16)) + cast(t2.redpackage as decimal(32,16))) AS money 
...
DROP TABLE IF EXISTS report_check.tmp_success_transferStatus;
