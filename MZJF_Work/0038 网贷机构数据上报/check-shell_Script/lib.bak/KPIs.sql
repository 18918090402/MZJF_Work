...
SELECT t2.sourceCode, t2.sourceProductCode, t2.userIdcardHash, t2.amount FROM last_online_status_1 t1 LEFT JOIN tmp_scatterinvest t2 ON t1.sourceProductCode = t2.sourceProductCode WHERE t2.sourceProductCode IS NOT NULL;

CREATE TEMPORARY TABLE loan_all_amount
select t1.sourceCode,t1.sourceProductCode,t1.userIdcardHash, (t1.amount + IFNULL(t2.curInterest, 0)) AS amount from tmp_lend_amount t1 left join (select sourceCode, sourceProductCode, userIdcardHash, IFNULL(sum(curInterest), 0) curInterest from tmp_repayplan group by sourceCode,sourceProductCode, userIdcardHash) t2 on t1.sourceProductCode = t2.sourceProductCode;

CREATE TEMPORARY TABLE loan_all_amount_19
SELECT t1.sourceCode,IFNULL(SUM(t2.transMoney), 0) transMoney FROM loan_all_amount t1 LEFT JOIN (SELECT * FROM tmp_transact WHERE transType = '19') t2 ON t1.sourceProductCode = t2.sourceProductCode;

CREATE TEMPORARY TABLE loan_all_amount_18
SELECT t1.sourceCode, IFNULL(SUM(t2.transMoney), 0) transMoney FROM loan_all_amount t1 LEFT JOIN (SELECT * FROM tmp_transact WHERE transType = '18') t2 ON t1.sourceProductCode = t2.sourceProductCode;
...
SELECT t1.*, IFNULL(t2.amount,0) amount FROM creditor_transfer t1 LEFT JOIN (SELECT * FROM tmp_transferStatus WHERE transferStatus = '2') t2
ON t1.transferId = t2.transferId;

CREATE TEMPORARY TABLE creditor_transfer_status_transact
SELECT t1.*, IFNULL(t2.transMoney,0) transMoney FROM creditor_transfer_status t1 LEFT JOIN (SELECT * FROM tmp_transact WHERE transType = '8') t2
...
SELECT t1.*, IFNULL(t2.amount,0) amount FROM undertake_transfer t1 LEFT JOIN (SELECT * FROM tmp_transferStatus WHERE transferStatus = '2') t2
ON t1.transferId = t2.transferId;

CREATE TEMPORARY TABLE undertake_transfer_status_transact
SELECT t1.*, IFNULL(t2.transMoney,0) transMoney FROM undertake_transfer_status t1 LEFT JOIN (SELECT * FROM tmp_transact WHERE transType = '8') t2
...
(SELECT userIdcardHash FROM undertake_transfer_status_transact WHERE (takeAmount - amount - transMoney) > 0.1
UNION 
SELECT userIdcardHash FROM creditor_transfer_status_transact WHERE (invAmount - amount - transMoney) > 0.1) AS t1) AS t3, key_t AS t2, num_of_borrowers AS t4;
