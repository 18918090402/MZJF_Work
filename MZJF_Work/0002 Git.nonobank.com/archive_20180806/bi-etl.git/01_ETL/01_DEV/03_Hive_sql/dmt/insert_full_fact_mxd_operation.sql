truncate table DMT.FACT_MXD_OPERATION;
set hive.exec.dynamic.partition.mode=nonstrict;
    
-- 1 用户注册
insert into DMT.FACT_MXD_OPERATION partition
  (PD)
  select SUBSTR(ui.REGISTER_TIME,1,10) TAG_DATE,
         ...
         SUBSTR(ui.REGISTER_TIME, 1, 7) PD
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(ui.STU_AUTH_TIME,1,10) TAG_DATE,
         ...
         'SYS' DW_UPDATE_BY,
         from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         SUBSTR(ui.STU_AUTH_TIME, 1, 7) PD
    from IDW.FACT_USER_INFO ui
   where ui.IS_STUDENT = 1
     and ui.P_TYPE = '名校贷'
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(bp.bp_publish_time, 1, 10) TAG_DATE,
         ...
         'SYS' DW_UPDATE_BY,
         from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         SUBSTR(bp.bp_publish_time, 1, 7) PD
    from odsopr.BORROWS_PREPARE bp
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(bo.VIDEO_AUDIT_TIME,1,10) TAG_DATE,
         ...
         'SYS' DW_UPDATE_BY,
         from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         SUBSTR(bo.VIDEO_AUDIT_TIME, 1, 7) PD
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(bo.BO_AUDIT_TIME,1,10) TAG_DATE,
         ...
         'SYS' DW_UPDATE_BY,
         from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         SUBSTR(bo.BO_AUDIT_TIME, 1, 7) PD
    from IDW.FACT_BORROWS bo
    join IDW.FACT_USER_INFO ui
   where bo.user_id = ui.user_id
     and bo.P_ID IN (77, 83, 87, 88)
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(br.BR_TIME, 1, 10) TAG_DATE,
         ...
         case
           when IS_FIRST_REPAY_OVERDUE = 1 then
            BR_PRICE_B
           else
            0
         end FIRST_REPAY_OVERDUE_B,
         case
           when IS_FIRST_REPAY_OVERDUE = 1 and BR_IS_REPAY = 0 then
            1
           else
            0
...
insert into DMT.FACT_MXD_OPERATION partition
  ...
  select SUBSTR(br.BR_REPAY_TIME, 1, 10) TAG_DATE,
         ...
         'SYS' DW_UPDATE_BY,
         from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         SUBSTR(br.BR_REPAY_TIME, 1, 7) PD
    from IDW.FACT_BORROWS_REPAYMENT br
    join IDW.FACT_BORROWS bo
      on br.bo_id = bo.bo_id
     and bo.p_id IN (77, 83, 87, 88)
