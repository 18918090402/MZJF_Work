set mapreduce.job.name = IDW_MXD_FACT_MXD_NC_DETAIL;
set mapreduce.job.queuename = etl-dw;
set hive.exec.dynamic.partition.mode=nonstrict;

 /*催收相关信息*/
 DROP TABLE IF EXISTS TMP.TMP_DUNNING_BO_ID;
 CREATE TABLE TMP.TMP_DUNNING_BO_ID AS
 SELECT BO_ID
 ,CONCAT(FROM_UNIXTIME(UNIX_TIMESTAMP() - 1 * 24 * 60 * 60, 'yyyy-MM'),'-01 06:00:00') AS start_time
 ,mod_contract
 ,odc_name
 ,mod_start_time
...
AND mod_contract=CONCAT('NC',FROM_UNIXTIME(UNIX_TIMESTAMP() - 1 * 24 * 60 * 60, 'yyyyMM'),'01')
;

/*提前还款违约金*/
DROP TABLE IF EXISTS TMP.TMP_PREPAYMENT_PENALTY;
CREATE TABLE TMP.TMP_PREPAYMENT_PENALTY AS
SELECT t1.borrows_id AS bo_id, MAX(penalty_fee_amount) AS amount
  FROM TMP.TMP_DUNNING_BO_ID t
...
insert overwrite table DMT_MXD.FACT_MXD_NC_DETAIL partition(stat_date,stat_hour)
...
,br1.start_overdue_price --截止取数时间的逾期总额
...
,CONCAT(cast(ROUND((NVL(br1.mod_price,0)/start_overdue_price)*100,2) as string),'%') AS overdue_rate
...
INNER JOIN TMP.TMP_DUNNING_BO_ID du ON bo.id=du.bo_id
...
LEFT JOIN TMP.TMP_PREPAYMENT_PENALTY fl ON bo.id=cast(fl.bo_id as bigint)
