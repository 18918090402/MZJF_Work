set mapreduce.job.name = DMT_MAIYA_FACT_MY_WW_DETAIL;
set mapreduce.job.queuename = etl-dw;
set hive.exec.dynamic.partition.mode=nonstrict;

/*催收相关信息*/
 DROP TABLE IF EXISTS TMP.TMP_DUNNING_MY_WW_BO_ID;
 CREATE TABLE TMP.TMP_DUNNING_MY_WW_BO_ID AS
 SELECT BO_ID
...
/*提前还款违约金*/
DROP TABLE IF EXISTS TMP.TMP_PREPAYMENT_PENALTY_MY_WW;
CREATE TABLE TMP.TMP_PREPAYMENT_PENALTY_MY_WW AS
SELECT t1.borrows_id AS bo_id, MAX(penalty_fee_amount) AS amount
  FROM TMP.TMP_DUNNING_MY_WW_BO_ID t
...
insert overwrite table DMT_MAIYA.FACT_MY_WW_DETAIL partition(stat_date)
...
,br1.unrepay_price --剩余还款金额
,br1.unrepay_price_punish --罚息和违约金
,br1.unrepay_price_b --未还本金
,fa.balance --可用余额
,fa.locking --锁定金额
...
INNER JOIN TMP.TMP_DUNNING_MY_WW_BO_ID du ON bo.id=du.bo_id
...
LEFT JOIN TMP.TMP_PREPAYMENT_PENALTY_MY_WW fl ON bo.id=cast(fl.bo_id as bigint)
...
INNER JOIN TMP.TMP_DUNNING_MY_WW_BO_ID du 
