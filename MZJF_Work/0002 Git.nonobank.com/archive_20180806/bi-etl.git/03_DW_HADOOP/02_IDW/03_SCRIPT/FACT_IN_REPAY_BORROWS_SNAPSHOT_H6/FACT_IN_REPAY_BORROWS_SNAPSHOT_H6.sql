set mapreduce.job.name = idw_fact_in_repay_borrows_snapshot_h6_01;
set mapreduce.job.queuename = etl-dw;
set hive.exec.dynamic.partition.mode=nonstrict;
set hive.exec.parallel=true;

insert overwrite table idw.FACT_IN_REPAY_BORROWS_SNAPSHOT_H6 partition
  (STAT_DATE)
  select bo.BO_ID,
         bo.P_ID,
         bo.BUSI_LINE,
    ...
    join (select br.bo_id,
                 ...
                 sum(case
                       when br.br_time < add_months('{T}', 1) then
                        1
                       else
                        0
                     end) POSTED_BILL_NUM,
