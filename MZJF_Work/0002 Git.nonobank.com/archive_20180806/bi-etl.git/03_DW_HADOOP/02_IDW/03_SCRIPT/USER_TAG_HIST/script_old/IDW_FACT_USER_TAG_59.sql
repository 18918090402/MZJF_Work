...
set mapreduce.job.name=IDW_FACT_USER_TAG_59_01;
...
FROM 
(
  select user_id, sum(price_principal) CSYY_7DAYS_ETA_AMT
   ...
   where plan_cash_time >= FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyy-MM-dd')
     ...
     and PRODUCT_CATEGORY <> '开心财神'
     and cash_status = 0
   group by user_id
) OUTPUT
INSERT OVERWRITE TABLE IDW.USER_TAG_TMP_01 PARTITION(TAG_ID=181) SELECT  USER_ID,CSYY_7DAYS_ETA_AMT, 4
;
-- -------------------------------------------------------------------------------------------------
set mapreduce.job.queuename=etl-dw;

set mapreduce.job.name=IDW_FACT_USER_TAG_59_02;
