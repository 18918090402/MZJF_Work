...
insert overwrite table idw.FACT_MAIYA_ORDER
  select 'SYS' AS DW_CREATE_BY,
         ...
         case
           when oo.ORDER_END_DATE > oo.create_time then
            oo.ORDER_END_DATE
         end ORDER_END_DATE,
         oc2.PASS_TIME,
         ...
         case
           when oo.LENDING_TIME > oo.create_time then
            oo.LENDING_TIME
         end LENDING_TIME,
         oo.ORDER_TYPE,
         oo.LBS_Y,
    ...
    left join (select o.order_id,
                 ...
                 from (select t1.order_id,
                              ...
                              t1.operator_type,
                              t2.reason
                         from (select order_id, status, operator_type, reason
                                 from ods.t_ord_order_check_history_hist
                                where status in (14, 15)) t1 Lateral View Explode(Split(t1.reason, ',')) t2 as reason
                        where t2.reason <> '') o
