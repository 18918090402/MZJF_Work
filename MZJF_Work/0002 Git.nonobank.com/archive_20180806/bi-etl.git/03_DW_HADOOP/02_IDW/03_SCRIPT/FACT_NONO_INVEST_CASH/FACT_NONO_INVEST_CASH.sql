...
insert overwrite table IDW.FACT_NONO_INVEST_CASH partition
  ...
  SELECT va.id CASH_SRC_ID, -- 精选正常兑付
         ...
         IF(fp.expect_unit = 0,
            ...
            DATE_ADD(fp.start_date, fp.expect)) PLAN_CASH_TIME,
         CASE
...
set mapreduce.job.name = idw_fact_nono_invest_cash_curing;
set mapreduce.job.queuename = etl-dw;
set hive.exec.parallel=true;
set hive.exec.dynamic.partition.mode = nostrict;
set hive.exec.dynamic.partition = true;
...
insert overwrite table IDW.FACT_NONO_INVEST_CASH partition
  (DATA_STATE)
  SELECT da.id CASH_SRC_ID,
         'debt_accept' CASH_SRC_TAB,
         da.user_id USER_ID,
         NULL FP_KEY,
         NULL FP_ID,
         NULL BO_ID,
         ...
         NULL CATEGORY_CODE_02,
         NULL CATEGORY_NAME_02,
         da.dac_time PLAN_CASH_TIME,
         da.dac_success_time ACTUAL_CASH_TIME,
         da.dac_is_pay CASH_STATUS,
         da.dac_price_b PRICE_PRINCIPAL,
         da.dac_price_l PRICE_INTEREST,
         NULL PRICE_AMT,
         da.dac_price_punish PRICE_PUNISH,
         1 CASH_TYPE_CODE,
         4 INVEST_TYPE_CODE,
         CONCAT_WS('_',
                   '03',
                   CAST(da.user_id AS STRING),
                   CAST(da.dt_id AS STRING)) INVEST_RELATION_NO,
         'SYS' DW_CREATE_BY,
         FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyy-MM-dd HH:mm:ss') DW_CREATE_TIME,
         'SYS' DW_UPDATE_BY,
         FROM_UNIXTIME(UNIX_TIMESTAMP(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME,
         ...
         null NEXT_VA_ID,
         'curing' DATA_STATE
    FROM ods.t_debt_accept_hist da
  union all
  SELECT frf.id CASH_SRC_ID,
         'fund_redeem_form' CASH_SRC_TAB,
         frf.user_id USER_ID,
         NULL FP_KEY,
         NULL FP_ID,
         NULL BO_ID,
         ...
         NULL CATEGORY_NAME_02,
         NULL PLAN_CASH_TIME,
         frf.create_time ACTUAL_CASH_TIME,
         1 CASH_STATUS,
         NULL PRICE_PRINCIPAL,
         NULL PRICE_INTEREST,
         frf.frf_amount PRICE_AMT,
         NULL PRICE_PUNISH,
         1 CASH_TYPE_CODE,
         5 INVEST_TYPE_CODE,
         ...
         null NEXT_VA_ID,
         'curing' DATA_STATE
    FROM ods.t_fund_redeem_form_hist frf
   WHERE frf.type = 1
     AND frf.frf_redeem_status <> 2;