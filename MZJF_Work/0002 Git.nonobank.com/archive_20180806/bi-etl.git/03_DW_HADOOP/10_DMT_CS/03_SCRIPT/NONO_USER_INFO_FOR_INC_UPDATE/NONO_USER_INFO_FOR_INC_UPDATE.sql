...
set mapreduce.job.name=DMT_CS_NONO_USER_INFO_FOR_INC_UPDATE_01;

INSERT OVERWRITE TABLE DMT_CS.NONO_USER_INFO_FOR_INC_UPDATE
  select USER_ID,
         USER_NAME,
    ...
    from (SELECT t1.USER_ID,
                 ...
                 NVL(MAX(CASE WHEN TAG_ID=40 THEN CAST(TAG_VALUE AS STRING) END),NULL) AS CHANNEL_NAME,
                 NVL(MAX(CASE WHEN TAG_ID=80 THEN CAST(TAG_VALUE AS DECIMAL(38,10)) END),NULL) AS NONO_MAX_AUM,
                 NVL(MAX(CASE WHEN TAG_ID=79 THEN CAST(TAG_VALUE AS DECIMAL(38,10)) END),NULL) AS NONO_AUM,
            ...
            join ods.t_user_info_hist t2
              on t1.user_id = t2.id
           WHERE TAG_ID in (2, 3, 4, 186, 46, 90, 20, 8, 41, 40, 80, 79, 64, 185, 14)
