...
select id,
  ...
  from (select id,
         ...
         where fl.operation_relation in (59, 184, 702, 689)
           and to_date(fl.create_day) <= '2017-12-31') t
 where t.bo_id <> ''
...
select bo_id, operation_relation, create_time, '2017-12-31' stat_date
  from (select bo_id,
               operation_relation,
               create_time,
          ...
          from (select case
                 ...
                 where fl.operation_relation in (151, 209)
                   and to_date(fl.create_time) >= to_date(date_add(date_sub(trunc('2017-12-31','YEAR'),1),1))
                   and to_date(fl.create_time) <= '2017-12-31'
                   and fl.id <> 1630026343
                union all
                select bo_id, 151 operation_relation, update_time create_time
                  from odsopr.borrows_consulted_fee
                 where zx_state = 2
                   and to_date(create_time) >= to_date(date_add(date_sub(trunc('2017-12-31','YEAR'),1),1))
                   and to_date(create_time) <= '2017-12-31'
                union all
                select bo_id, 
                       case
