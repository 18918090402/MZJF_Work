...
insert overwrite table IDW.FACT_NONO_INVEST_CASH partition (DATA_STATE)
-- 散投、债转正常结算
  SELECT ba.id CASH_SRC_ID, 
...
SELECT  va.id CASH_SRC_ID
        ...
        ,CASE WHEN fl2.create_time IS NULL THEN va.cash_time ELSE fl.create_time END ACTUAL_CASH_TIME
        ,va.is_cash CASH_STATUS
        ,case
            when fp.scope <> 26 and va.amount > coalesce(debt.debt_amt, 0) then (va.amount - coalesce(debt.debt_amt, 0))
            when fp.scope = 26 then (va.amount - coalesce(qf.quit_amount, 0))
            else 0
        end PRICE_PRINCIPAL
        ,null PRICE_INTEREST
        ,null PRICE_AMT
        ...
        ,1 CASH_TYPE_CODE
        ,3 INVEST_TYPE_CODE
        ,concat_ws('_', '02', cast(va.user_id as string), cast(va.fp_id as string)) INVEST_RELATION_NO
        ,'SYS' DW_CREATE_BY
        ,from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_CREATE_TIME
        ,'SYS' DW_UPDATE_BY
        ,from_unixtime(unix_timestamp(), 'yyyy-MM-dd HH:mm:ss') DW_UPDATE_TIME
        ...
        ,va.platform
        ,va.id VA_ID
        ,case when fp.scope = 28 then ict.locking_expect else fp.expect end EXPECT
        ,CASE WHEN fp.expect_unit = 0 THEN 2 WHEN fp.expect_unit = 1 THEN 1 END EXPECT_UNIT_CODE
        ,va.reinvest_type REINVEST_TYPE
        ,rt.expect REINVEST_EXPECT
        ,CASE 
            WHEN rt.expect_unit = 0 THEN 2
            WHEN rt.expect_unit = 1 THEN 1
...
left join (
            select  ds.va_id, sum(dbl.price) debt_amt
       ...
       on va.id = debt.va_id
left join (
            select va_id, sum(quit_amount) quit_amount
