     insert overwrite table dw_srm.fact_risk_vintage_quota_m partition (STAT_DATE)
            ...
            (     
                 select  br.bo_id                                                                                                              as bo_id
                        ...
                        ,if((bo2.user_id is null), if(bo3.rn = 1, '0','1'), if(bo2.user_type = '0', '0', '1'))                                 as user_type
                        ,case 
                        ...
                        ,max(
                            if((br.br_is_repay = 0 or to_date(br.br_repay_time) > '{T_SUB_1}') and br.br_time <= '{T_SUB_1}'
                                ,datediff(if(to_date(br_repay_time) <= '{T_SUB_1}', to_date(br_repay_time), date_add('{T_SUB_1}', 1))
                                ,br.br_time)
                            ,0)
                            )                                                                                                                  as overdue_unrepay_days
                   ...
                   join idw.fact_borrows bo
                    ...
                    left join (
                                select bo.user_id, bo.bo_id,tv.user_type as user_type
                                from idw.fact_borrows bo
                                left join dw_srmopr.fact_risk_vintage_quota_m tv
                                    on tv.bo_id = bo.bo_id
                                where bo.bo_is_success = 1
                                   and to_date(tv.stat_date) <= to_date(date_sub(trunc('{T_SUB_1}','MM'), 1)) 
                                   and to_date(tv.stat_date) > to_date(date_sub(trunc('{T_SUB_1}','MM'), 2))  --上月
                                group by bo.user_id
                                        ,bo.bo_id
                                        ,tv.user_type
                        ) bo2 
                     on bo2.bo_id = bo.bo_id
                        -- and bo2.user_id = bo.user_id
                      left join (
                                  select bo.user_id, bo.bo_id
                                         ,row_number() over(partition by bo.user_id order by bo.bo_id) as rn
                                  from idw.fact_borrows bo 
                                  where bo.bo_is_success = 1
                                     and to_date(bo.bo_success_time) <= '{T_SUB_1}'  --本月数据
                                     and to_date(bo.bo_success_time) >= to_date(date_sub(trunc('{T_SUB_1}','MM'), 1))
                               

                        ) bo3
                      on bo3.bo_id = bo.bo_id
                    where bo.bo_is_success = 1 
                 ...
                 select  br.bo_id                                                                                                              as bo_id
                        ...
                        ,bo.p_key                                                                                                              as p_key
                           ,case 
                               ...
                               when bo1.flag = '新医美' and (bo1.qd_pass <> '1' or bo1.qd_pass is null )   then '020201'
                        ...
                        ,max(
                            if((br.br_is_repay = 0 or to_date(br.br_repay_time) > '{T_SUB_1}') and br.br_time <= '{T_SUB_1}'
                                ...
                                ,br.br_time)
                            ,0)
                            )                                                                                                                  as overdue_unrepay_days   --当前逾期未还天数***                                                                                                      
                        ,sum(case when br.br_is_repay = 0 or to_date(br.br_repay_time) > '{T_SUB_1}' then br.br_price_b else 0 end)            as unrepay_price_b
                        ,'{T_SUB_1}'                                                                                                           as stat_date
                   from idw.fact_borrows_repayment br
                   join idw.fact_borrows bo
              ...
              left join (
                    ...
                    left join (
                              ...
                              where bo.bo_is_success = 1
                                 and to_date(bo.bo_success_time) <= '{T_SUB_1}'  
                                 and to_date(bo.bo_success_time) >= to_date(date_sub(trunc('{T_SUB_1}','MM'), 1))--本月数据
                              -- group by bo.user_id
                              --       ,bo.bo_id
                 ...
                 left join (      
                              select ui.id user_id,
                                ...
                                left join (select t1.user_id, t2.degree, t2.auth_time, t2.graduate_date, t2.college
                                             from odsopr.user_loan_auth_info t1
                                             join odsopr.user_edu_auth_log t2
                                               on t1.auth_code = 1
