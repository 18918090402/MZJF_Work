...
if product in ("白领包","应届贷" ,"社保贷") then product_L="2-白领贷";
else if product in ("其他大额贷" ,"消费贷" ,"学历贷") then product_L="3-麦子借款";
else if product in ("本科包","名校贷应急包","专科包") then product_L="4-信用贷";
run;
...
if approve_result in ('终审通过（还未放款）' ,'终审通过已放款' ) and apply_no ne 0 
then do pass=1;
pass_amt=apply_amt;
end;
...
/*拒绝*/
if index(APPROVE_RESULT , '拒绝') ge 1 then refuse=1;
else refuse=.;
/*取消*/
...
help_Sum=申请件数
apply_amt_mean=申请件均
apply_term_avg=申请平均期限
refuse_sum=拒绝件数
refuse_ratio=拒绝率
apply_amt_weight_pass_Sum=申请通过金额
pass_sum=通过件数
pass_ratio=通过率
pass_amt_mean=申请通过件均
pass_apply_term_avg=申请通过平均期限
cancel_sum=取消件数
cancel_ratio=取消率
fraud_sum=欺诈件数
fraud_ratio=欺诈率;
format refuse_ratio pass_ratio cancel_ratio fraud_ratio percent8.1;
run;
