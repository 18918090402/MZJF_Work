...
insert overwrite table idw.fact_borrows
        SELECT bo.bo_id,
                 ...
                 bo.rc_name,
                 case
                   when ym.bo_id is not null and ym.is_lt = '否' then 
                    28
                   when ym.bo_id is not null and ym.is_lt = '是' then 
                    29 
                   when bo.p_id = 148 then 
                    30 
                   ...
                   when bo.p_id in (94, 97) then
                    18
                   when bo.bo_audit_time < '2016-06-29' then
                    16
                   else
                    19
                 end p_key,
            ...
            (
                  SELECT t1.id AS bo_id, --  借款ID
                         ...
                         CASE WHEN LENGTH(t1.bo_title) < 5 AND t1.bo_title NOT REGEXP '^[0-9A-Za-z]*$' THEN t1.bo_title
                              ...
                              ELSE '(未知)'
                         END AS bo_purpose,
                         CASE WHEN t1.bo_is_success = 1 AND t1.bo_all_repayed = 1 THEN '还款完成'
                         ...
                         ba.video_audit_time,
                         CASE WHEN ba.bo_id IS NOT NULL AND (ba.is_audit = 0 OR ba.is_audit IS NULL) THEN '未提交'
                         ...
                         END AUDIT_STATUS,
                         CASE WHEN ba.bo_id IS NOT NULL AND (ba.is_confirm = 0 OR ba.is_confirm IS NULL) THEN '未提交'
    ...
    LEFT JOIN tmp.tmp_fact_borrows_01 tmp3
           ON bo.bo_id = tmp3.bo_id
    LEFT JOIN 
            (
                       SELECT bp_id, SUM(cutvalue) interest_rate_cut
                         FROM odsopr.user_interestratecut
                        WHERE bp_id > 0
                        GROUP BY bp_id
            ) urc
    ...
    LEFT JOIN idw.DIM_ASSET_PRODUCT p
           ON bo.p_id = p.p_id
    LEFT JOIN odsopr.borrows_snapshot bs
    ...
    left join 
            (
                  select ui.id USER_ID,
                         ...
                         case when usi.EDU_GRADUATE_DATE regexp '^(\\d)*$' and length(usi.EDU_GRADUATE_DATE) = 8 then 
                                    concat(substr(usi.EDU_GRADUATE_DATE, 1, 4), '-', substr(usi.EDU_GRADUATE_DATE, 5, 2), '-', substr(usi.EDU_GRADUATE_DATE, 7, 2))
                              when usi.EDU_GRADUATE_DATE regexp '^(\\d){4}\\-(\\d){2}\\-(\\d){2}$' then usi.EDU_GRADUATE_DATE
                         end as edu_graduate_date
            ...
            ) ui     --20190305  直接用 fact_user_info 的逻辑
           on bo.user_id = ui.user_id
           
    left join odsopr.fact_channel_group cg
    ...
    left join msc.xinyimei ym 
           on cast(ym.bo_id as bigint) = bo.bo_id 
          and cast(ym.nono_id as bigint) = bo.user_id
              ;