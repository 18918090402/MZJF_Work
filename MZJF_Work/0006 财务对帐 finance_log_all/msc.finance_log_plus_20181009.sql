...
(
    ...
    ,income                 decimal(38,10)
    ,expend                 decimal(38,10)
    ,create_time            string
)
COMMENT '资金日志升级版'
...
--tmp DDL
drop table if exists msc.tmp_finance_log_plus;

create table msc.tmp_finance_log_plus
(
    finance_ind             string
    ,category               string          --投资        or   借款
    ,type                   string          --nono/csyy   or   biz_name
...
-------------------------------------------------------------------------
set mapreduce.job.name=msc_tmp_finance_log_plus_1; 
...
-- 01 利息发放 不区投资借款
insert overwrite table msc.tmp_finance_log_plus
select 
    '01' finance_ind
    ,NULL category
    ,NULL type
...
-- 02 归属于 nono 资金 
insert into table msc.tmp_finance_log_plus
select 
    '02' finance_ind
    ,'资金' category
    ,'nono' type
...
where t.operation_relation in ( 714,722,735,738,740,742,753,  812,815,818,822,702, 848 )
;
...
-------------------------------------------------------------------------
set mapreduce.job.name=msc_tmp_finance_log_plus_2; 
set mapreduce.job.queuename = etl-dw;  
set hive.exec.parallel=true ;
set hive.groupby.skewindata = true ;
set hive.map.aggr = true ; 
...
and t.id not in (1672056447, 1675850419)    --代偿数据  存管上线时，徽商有不明确的交易单码，人工修复的数据 不应该计入日志表，其他业务已经删除
;


-- 03   731          
drop table if exists msc.tmp_finance_log_plus1_731;

create table msc.tmp_finance_log_plus1_731
stored as parquet 
as
select 
...
select 
    ...
    ,expend
    ,create_time
from msc.tmp_finance_log_plus1_731 t
where t.bo_id is not null
;


drop table if exists msc.tmp_finance_log_plus2_731;

create table msc.tmp_finance_log_plus2_731
stored as parquet 
as
select 
    '03_proof_id' finance_ind
...
select 
    ...
    ,expend
    ,create_time 
from msc.tmp_finance_log_plus2_731 t
where t.bo_id is not null
;
...
-------------------------------------------------------------------------
set mapreduce.job.name=msc_tmp_finance_log_plus_3; 
set mapreduce.job.queuename = etl-dw;  
set hive.exec.parallel=true ;
set hive.groupby.skewindata = true ;
set hive.map.aggr = true ; 
...
-- 03
drop table if exists msc.tmp_finance_log_plus1;

create table msc.tmp_finance_log_plus1
stored as parquet 
as
select 
    '03_proof_id' finance_ind 
...
select 
    ...
    ,expend
    ,create_time
from msc.tmp_finance_log_plus1 t
where t.bo_id is not null
;
...
--03 
/*
drop table if exists msc.tmp_finance_log_plus2;
create table msc.tmp_finance_log_plus2
stored as parquet 
as
select 
    '03_proof_id' finance_ind
...
select 
    ...
    ,expend
    ,create_time
from msc.tmp_finance_log_plus2 t
where t.bo_id is not null
;
...
/*
--03
drop table if exists msc.tmp_finance_log_plus4;
create table msc.tmp_finance_log_plus4
stored as parquet 
as
select 
    '03_proof_id' finance_ind
    ...
    ,t.expend
    ,t.create_time
from msc.tmp_finance_log_plus3 t
left join odsopr.acc_depository_trans_record t1
on t.proof_id = t1.proof_id
inner join odsopr.fund_trans_repayment_order t2 
on t1.trd_no = t2.trans_id
...
select 
    ...
    ,expend
    ,create_time
from msc.tmp_finance_log_plus4 t
where t.bo_id is not null
;
...
-------------------------------------------------------------------------
set mapreduce.job.name=msc_tmp_finance_log_plus_4; 
set mapreduce.job.queuename = etl-dw;  
set hive.exec.parallel=true ;
set hive.groupby.skewindata = true ;
set hive.map.aggr = true ; 
...
--  04    751,681,683,771,692,694,696,745,808    (9 id 重新梳理)
...
--04 资产
drop table if exists msc.tmp_finance_log_invest;

create table msc.tmp_finance_log_invest
stored as parquet 
as
...
(
    ...
    where operation_relation in (751,   771,694,696,681,683,692,745,808)
) t  
where t.rn = 1 
;


drop table if exists msc.tmp_finance_log_loan;

create table msc.tmp_finance_log_loan
stored as parquet 
as
...
(
    ...
    left join odsopr.finance_account b
    on b.id = a.opposite_account_id
    left join msc.tmp_finance_log_invest c   
    on c.id = a.id
    and c.operation_relation = a.operation_relation
    left join msc.tmp_finance_log_loan d
    on d.id = a.id
...
(
    ...
    left join odsopr.finance_operation_relation f
    on a.operation_relation = f.id
    where a.operation_relation in (771,694,696,681,683,692,745,808)
) tt
;


-------------------------------------------------------------------------
set mapreduce.job.name=msc_tmp_finance_log_plus_5; 
set mapreduce.job.queuename = etl-dw;  
set hive.exec.parallel=true ;
set hive.groupby.skewindata = true ;
set hive.map.aggr = true ; 
...
-------------------------------------------------------------------------
set mapreduce.job.name=msc_finance_log_plus; 
set mapreduce.job.queuename = etl-dw;  
set hive.exec.parallel=true ;
set hive.groupby.skewindata = true ;
set hive.map.aggr = true ; 
...
insert overwrite table msc.finance_log_plus
select * from msc.tmp_finance_log_plus
;
